...
`run_coding.py` assembles pass B's input from pass A's output.

There are **four blocks**, plus a closing that is not a block:
...
In both arms the interview's **first question is asked outside the chat** (in Qualtrics), so the first participant turn is already an answer and has no opening AI question before it.
...
Sort by `messageID` in ascending order. Number **only the participant turns** sequentially, starting from 1. That index is what pass A returns and what pass B uses.
...
6. **Blocks can be missing.** Interviews break off early. If the interviewer never reached a block, that block has no turns. 
7. Off-topic turns, "no sé", and one-word noise ("Me", "ok") still get the block that was open.

### Output (pass A)

Return **only** a JSON object. One entry per participant turn, `p` matching the `[Pn]` index, every turn present exactly once including noise turns.
...
- `*_evidence` -- verbatim quote(s) from the respondent, in Spanish. Empty string for `no_answer`. Never paraphrase, never quote the interviewer.
